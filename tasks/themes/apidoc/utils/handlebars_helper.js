define([
    ...
    'diffMatchPatch',
    'lodash'
], function(locale, Handlebars, DiffMatchPatch, _) {
    ...
    Handlebars.registerHelper('var', function(name, value, context) {
        context.data.root.article[name] = value;
        this[name] = value;
    });

    Handlebars.registerHelper('selectedInput', function(value, reference, context) {
        // default statement
        return value === context.data.root.article[reference] ? 'selected="selected"' : '';
    });

    Handlebars.registerHelper('QueryParametersToTemplate', function(host, endpoint, context) {
        // normalize this value first
        var value = _.compact([ host, endpoint ]).join('/').replace(/\/+/, '/');
        // try to match parameters
        var match = value.match(/(:\w+)/g) || [];

        // only on this case
        if (!_.isEmpty(match)) {
            match = _.map(match, function (m) {
                return m.replace(/:+/g, '');
            });
        }
